...
class ExportRequest(BaseModel):
    """Export kérés – job azonosítóval és opcionális fájlszűréssel"""
    job_id: str
    file_ids: Optional[List[str]] = None    # Ha None, az összes fájl exportálódik


def get_results_for_export(job_id: str, file_ids: Optional[List[str]]):
    """
    A megadott job eredményeinek lekérése exportáláshoz.
    Opcionálisan szűrhető fájl ID-k alapján.
    """
    ...
    # Opcionális szűrés fájl ID-k alapján
    if file_ids:
...
async def export_pdf(request: ExportRequest):
    ...
    results = get_results_for_export(request.job_id, request.file_ids)
